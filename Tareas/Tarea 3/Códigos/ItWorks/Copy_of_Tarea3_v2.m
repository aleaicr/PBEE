...
for i = 1:n_ests                                                            % Para cada estructura i
    ...
    % Media y Desviación logarítmica (logN)
    syms muln stdln
    muln_min = min(frag(i).all_IM);
    muln_max = max(frag(i).all_IM);
    muln_step = 1/50*(muln_max-muln_min);
    muln_vect = (muln_min:muln_step:muln_max).';
    stdln_min = 0.1;
    stdln_max = 0.99;
    stdln_step = 0.05;
    stdln_vect = (stdln_min:stdln_step:stdln_max).';

    ver_anterior = -inf;                                                    % Auxiliar
    for mu = 1:length(muln_vect)                                            % Probamos para muchos muln
        m = muln_vect(mu);
        for sdesv = 1:length(stdln_vect)                                              % Probamos para muchos stdln
            s = stdln_vect(sdesv);
            for im = 1:length(frag(i).all_IM)                               % Obtenemos la tabla para todas las IM = im
                frag(i).Pc(im,1) = normcdf((log(frag(i).all_IM(im,1)) - m)./s); % Probabilidad de colapso
                frag(i).NPc(im,1) = 1 - frag(i).Pc(im,1);
                frag(i).Pbinom(im,1) = nchoosek(frag(i).nj_sim(im,1),frag(i).zj_col(im,1))*(frag(i).Pc(im,1).^frag(i).zj_col(im,1))*(frag(i).NPc(im,1)).^(frag(i).nj_sim(im,1) - frag(i).zj_col(im,1)); % P Binomial
            end
            % Verosimilitud
            ver = exp(sum(frag(i).Pbinom(:,1)));                            % Pitatoria() = exp(log(pitatoria)) = exp(sumatoria)
            % Verificamos si esta es la máxima verosimilitud
            if ver > ver_anterior
                frag(i).max_ver = ver;
                frag(i).muln = m;
                frag(i).stdln = s;
            end % else, siguiente combinación hasta encontrar la que de la máxima verosimilitud
            ver_anterior = ver;
        end
    end
    ...
    % Figuritas
    figure
    plot(frag(i).all_IM(:,1),logncdf(frag(i).all_IM(:,1),frag(i).muln,frag(i).stdln))
    xlabel('IM: Sa(T_1) [g]')
