...
% Fragility Curve
for i = 1:n_ests
    muln = mean(log(Data(i).IMc).');
    stdln = std(log(Data(i).IMc).');
    PC = normcdf((log(Data(i).IMc).'-muln)./stdln);
    IMc_range = (0.1:0.1:max(Data(i).IM)).';
    FC = normcdf(log(IMc_range)-muln./sigma);
