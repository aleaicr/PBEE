function [Sa]= Sa_or(GMFolder,GMDataName,T,xi,beta_newmark)
% Contreras - Sanguinetti
% Ingeniería Sísmica Avanzada - USM 2022

% Leer registros y guardar Sa(T1) original
...
while ischar(fLine)
    ...
    for i = 4:length(files)
        if string_data_line(1) == string(files(i).name)                     % Solo si es el mismo registro
            ...
            Sa(i-3,1) = Newmark_Lineal_Sa(beta_newmark,xi,a(i-3).dt,0,0,a(i-3).GM,T); % Guardar aceleración espectral del primer modo, GM está en m/s
        end
    end
